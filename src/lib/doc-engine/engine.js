...
class BookEngine {
  ...
  async init() {
    ...
    const load = async (name, fallbackStd) => {
      ...
      // Fallback to StandardFonts if custom font missing (Vercel or ENOENT)
      if (fallbackStd) {
        console.warn(`[BookEngine] Font ${name} not found, falling back to ${fallbackStd}`);
        return await this.doc.embedFont(StandardFonts[fallbackStd]);
      }
      throw new Error(`Font ${name} not found in any candidate: ${candidates.join(', ')}`);
    };

    // Selectable fonts — user can choose body/heading, with Lora/Playfair as default for backward compat
    const bodyFamily = this.book.fontBody || this.book.fonts?.body || 'Lora';
    const headingFamily = this.book.fontHeading || this.book.fonts?.heading || 'PlayfairDisplay';
    const fontMap = {
      Lora: { regular: 'Lora-Regular.ttf', bold: 'Lora-Bold.ttf', italic: 'Lora-Italic.ttf', boldItalic: 'Lora-BoldItalic.ttf', fallback: 'TimesRoman' },
      Merriweather: { regular: 'Merriweather-Regular.ttf', bold: 'Merriweather-Bold.ttf', italic: 'Lora-Italic.ttf', boldItalic: 'Lora-BoldItalic.ttf', fallback: 'TimesRoman' },
      Roboto: { regular: 'Roboto-Regular.ttf', bold: 'Roboto-Bold.ttf', italic: 'Lora-Italic.ttf', boldItalic: 'Lora-BoldItalic.ttf', fallback: 'Helvetica' },
      Inter: { regular: 'Inter-Regular.ttf', bold: 'Inter-Regular.ttf', italic: 'Lora-Italic.ttf', boldItalic: 'Lora-Italic.ttf', fallback: 'Helvetica' },
      SourceSerif4: { regular: 'SourceSerif4-Regular.ttf', bold: 'Lora-Bold.ttf', italic: 'Lora-Italic.ttf', boldItalic: 'Lora-BoldItalic.ttf', fallback: 'TimesRoman' },
      EBGaramond: { regular: 'EBGaramond-Regular.ttf', bold: 'Lora-Bold.ttf', italic: 'Lora-Italic.ttf', boldItalic: 'Lora-BoldItalic.ttf', fallback: 'TimesRoman' },
      CrimsonText: { regular: 'CrimsonText-Regular.ttf', bold: 'Lora-Bold.ttf', italic: 'Lora-Italic.ttf', boldItalic: 'Lora-BoldItalic.ttf', fallback: 'TimesRoman' },
    ...
    this.fonts = {
      ...
      bodyBoldItalic: await load(bodyFiles.boldItalic || bodyFiles.italic, bodyFiles.fallback === 'TimesRoman' ? 'TimesRomanBoldItalic' : 'HelveticaOblique'),
